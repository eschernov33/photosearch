...
class PhotoSearchDetailRepositoryImpl @Inject constructor(
    ...
    override suspend fun getPhotoById(photoId: Int, debugMode: Boolean): PhotoDetail? {
        if (debugMode) {
            delay(500)
            val photoItem = DebugData().getDebugPhotoById(photoId)
            return if (photoItem == null)
                null
            else
                mapper.mapPhotoApiItemToPhotoDetail(photoItem)
        }
        return try {
